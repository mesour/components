...
class Image
{
     ...
     * @param  string $file
     * @param  mixed $format detected image format
     * @throws Mesour\NotSupportedException if gd extension is not loaded
    ...
    public static function fromFile($file, & $format = NULL)
    {
        if (!extension_loaded('gd')) {
            throw new Mesour\NotSupportedException('PHP extension GD is not loaded.');
        }
        static $funcs = [
        ...
        return new static(Helpers::invokeSafe($funcs[$format], [$file], function ($message) {
     ...
     * @param $s
     * @return static
     * @throws Mesour\NotSupportedException
    ...
    public static function fromString($s)
    {
        ...
        return new static(Helpers::invokeSafe('imagecreatefromstring', [$s], function ($message) {
            throw new ImageException($message);
        }));
    }
    ...
    public static function fromBlank($width, $height, $color = NULL)
    {
        ...
        $height = (int)$height;
        if ($width < 1 || $height < 1) {
            throw new Mesour\InvalidArgumentException('Image width and height must be greater than zero.');
        }
     ...
     * @param  int $transparency transparency 0..127
     * @return array
     */
     ...
     * @param  int $quality quality 0..100 (for JPEG and PNG)
    ...
    public function save($file = NULL, $quality = NULL, $type = NULL)
    {
        ...
        switch ($type) {
            ...
            default:
                throw new Mesour\InvalidArgumentException('Unsupported image type \'$type\'.');
        }
    }
    ...
    protected function setImageResource($image)
    {
        if (!is_resource($image) || get_resource_type($image) !== 'gd') {
            throw new Mesour\InvalidArgumentException('Image is not valid.');
        }
        $this->image = $image;
        return $this;
    }
     ...
     * @param  mixed $srcWidth  source width
     * @param  mixed $srcHeight source height
     * @param  mixed $left      x-offset in pixels or percent
     * @param  mixed $top       y-offset in pixels or percent
    ...
    public static function calculateSize($srcWidth, $srcHeight, $newWidth, $newHeight, $flags = self::FIT)
    {
        ...
        if ($flags & self::STRETCH) { // non-proportional
            if (empty($newWidth) || empty($newHeight)) {
                throw new Mesour\InvalidArgumentException('For stretching must be both width and height specified.');
            }
            ...
            if (empty($newWidth) && empty($newHeight)) {
                throw new Mesour\InvalidArgumentException('At least width or height must be specified.');
            }
            $scale = [];
    ...
    public function __get($name)
    {
        ...
        throw new Mesour\InvalidArgumentException('Property ' . $name . ' does not exist.');
    }
